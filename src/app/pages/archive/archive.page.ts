...
export class ArchivePage implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.valdateSelect = await this.storage.get('valdateSelect');
    ...
    await this.activaterouter.params.subscribe(async (params:any) => {
      ...
      if(params['userNameThisConv']!="" && params['userNameThisConv']!=null && params['userNameThisConv']!=undefined && params['userNameThisConv']!=0){
        this.userNameThisConv = params['userNameThisConv'];
        await this.storage.set('userNameThisConv',this.userNameThisConv);
      }
      await this.functionReturnData(this.selectNumber);
    });
  }
async functionReturnData(numberUser:any): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    let sendValues = {
      ...
      dataSearch: numberUser,
      date:this.valdateSelect
    };
    this.chatService.chatSearch(sendValues)
      .then(data => {
        this.returnResultData = data;
        if (this.returnResultData.messageId === 1) {
  ...
  functionChats(number:any,name:any,chatSessionIdAdd:any,chatSessionIdAr:any){
    if(this.type == 1)
      this.navCtrl.navigateRoot(['/chatsarchive', {number:number,name:name,chatSessionId:chatSessionIdAdd,chatSessionIdAr:chatSessionIdAr,userNameThisConv:this.userNameThisConv}]);
    else{
      this.navCtrl.navigateRoot(['/chatsarchivenew', {number:number,name:name,chatSessionIdAr:chatSessionIdAr}]);
    }
  }
  functionBackHome(number:any,chatSessionId:any){
    if(this.type == 1)
      this.navCtrl.navigateRoot(['/chats', {number:number,chatSessionId:chatSessionId,userNameUsed:this.userNameThisConv,backUrl:1}]);
    else
